...
public class PassExample {
    // Static 변수들 - 메서드 영역에 저장
    static int callCount = 0;           // 메서드 호출 횟수 추적
    static String lastOperation = "";   // 마지막 수행된 작업
    static int[] sharedArray = {10, 20, 30}; // 모든 메서드가 공유하는 배열
    
    // Static 메서드 - 호출 추적
    static void trackCall(String methodName) {
        callCount++;
        lastOperation = methodName;
        System.out.println("[TRACK] " + methodName + " called. Total calls: " + callCount);
    }
    
    // Static 메서드 - 공유 배열 조작
    static void modifySharedArray(int index, int value) {
        trackCall("modifySharedArray");
    ...
    // 기존 메서드들에 추적 기능 추가
    static void changePrimitive(int x) {
        trackCall("changePrimitive");
        x = 100;
    }
    
    static void changeObject(int[] arr) {
        trackCall("changeObject");
        arr[0] = 999;
    }
    
    static void changeReference(int[] arr) {
        trackCall("changeReference");
        arr = new int[]{100, 200};
    }
    ...
    public static void main(String[] args) {
        ...
        // 최종 상태
        System.out.println("프로그램 종료 전 최종 상태:");
        printStatus();
    }
}
```
